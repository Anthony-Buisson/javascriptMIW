...
function affLigne(n) {
    ...
    document.forms[0].quantity[n].value = document.forms[0].product[n].value !== '' ?  1 : '';
}
function creerLigneProduit(index) {
    let div = _ce('div', _('.produits')[0]);
    let select = _cn('select', {name: 'product', onchange: affLigne(index)}, {}, div);
    for (let i = 0; i < libProd.length; i++) {
        let option = _cn('option', {value: prxProd[i]},{}, select);
        _ct(libProd[i], option);
    }
     _cn('input', {type: 'number', name: 'price', placeholder: 'Prix', readonly: true}, {}, div);
     _cn('input', {type: 'number', name: 'quantity', placeholder: 'Quantité', readonly: true}, {}, div);
     _cn('button', {type: 'button', onclick: plus(index)}, {}, div);
     _cn('button', {type: 'button', onclick: moins(index)}, {}, div);
     _cn('button', {type: 'button', onclick: sup(index)}, {}, div);
     _ce('br', div);
...
function afficheForm() {
    let ch = `<form name="formulaireCommande">
            ...
            <fieldset class="produits" onclick="affMontant()">
